...
float M1;             // Moisture sensors 1 (beware M2, M3, M4 does not work...)
...
void setup() {
  ...
  //  delay(1000);
  //  Serial.println("open");
  //  activateCoil(3, true);  //open coil3
  //  delay(2000);
  //  Serial.println("close");
  //  activateCoil(3, false); //close coil3
  ...
  // touch_pad_config(TOUCH_PAD_NUM0, 0); //T0 does not work (outputs 0 when motor driver is soldered)
  // touch_pad_config(TOUCH_PAD_NUM2, 0); //T2 does not work (outputs 0 when motor driver is soldered)
  // touch_pad_config(TOUCH_PAD_NUM3, 0); //T3 does not work (outputs 0 when motor driver is soldered)
  ...
  M1 = float(output);
  //  delay(100);
  //  touch_pad_read(TOUCH_PAD_NUM2, &output);  //T2 or M3
  //  M3 = float(output);
  //  delay(100);
  //  touch_pad_read(TOUCH_PAD_NUM3, &output);  //T3 or M4
  //  M4 = float(output);

  timeOut = millis();
  Serial.print("time internal: ");
  display_time();
}
      ...
      if (id == UNIT_MAC)
      {
        switch (index)
        {
          ...
          case 0xFF:  //time synchro
            sendStatetimeOut = 10000;  //decrease the timeout value
